...
{
	/* se aloca memorie pentru detaliile de pe campul "flow_duration"
	...
	while (traffic_curent[j] != ',') {
		/* in "traffic.flow_duration" se pastreaza valoril
		de pe campul "flow_duration" */
		traffic.flow_duration[j - i] = traffic_curent[j];
		j++;
	}
	// fprintf(tout, "%s", traffic.flow_duration);
	// fprintf(tout, "\n\n");
	...
	/* se aloca memorie pentru detaliile de pe campul
	...
	/* in "traffic.flow_pkts_payload" se pastreaza valoril
	de pe campul "traffic.flow_pkts_payload.avg" */
	for (int j = i; j < line_size - 1; j++)
		traffic.flow_pkts_payload[j - i] = traffic_curent[j];

	/* se verifica daca lungimea sirului de caractere de pe campul
	"flow_pkts_payload" este 3 și dacă acest sir este "0.0" */
	// fprintf(tout, "\n%s\n", traffic.flow_pkts_payload);
...
int main(void)
{
	...
	*fin = fopen("data/urls/urls.in", "r");
	// se aloca memorie pentru un URL
	...
	*tin = fopen("data/traffic/traffic.in", "r");
	...
	while (line_size >= 0) {
		result = 0;
		if (first_line != 1) {
			// fprintf(tout, "traffic=%s", traffic_curent);
			task2_part1();
			if (result == 1)
				fprintf(tout, "1\n");
			else
